...
function start-up(){
  ...
  # Normaly we would do this as a multi stage
  # build, however the dowloader puts the installer
  # in an unrecognized user/group which docker
  # can't change.
  ...
  docker run --rm \
    ...
    maven:3-openjdk-17 \
    /bin/sh -c "mvn org.apache.maven.plugins:maven-dependency-plugin:3.3.0:unpack -Dartifact=com.github.a-langer:org.eclipse.oomph.console.product:LATEST:tar.gz:linux.gtk.x86_64 -DoutputDirectory=./ -Dproject.basedir=./"

  # Fix the user/group problem outside docker
  chown -R root:root $(pwd)/bin && chmod -R +x $(pwd)/bin
